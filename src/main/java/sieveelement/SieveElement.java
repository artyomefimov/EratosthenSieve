...
public class SieveElement extends AbstractSieveElement // todo implement custom mailbox for sieve elements with ability to clear it
{
        ...
        return receiveBuilder()
                ...
                { // todo refactor
                    if (isWaitingForNextElement)
                        getSelf().tell(nextNumberMessage, getSelf());
                    else
                    {
                ...
                .match(GeneratedMaxActorsMessage.class, generatedMaxActorsMessage ->
                    log.info("Generation is ending. Last actor has number: " + currentNumber) // todo clear mailbox
                )
                .build();
    }
